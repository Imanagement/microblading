...
jQuery(document).on('click', '.stripe_slide', function(e){
	var current_slider = jQuery(this).parents('.stripe_gallery_wrapper');
	var slideObj = current_slider.attr('data-uniqid');
...
jQuery(document).ready(function ($) {	
	...
	if (jQuery('.stripe_gallery_trigger').size() > 0) {
		...
		jQuery('.stripe_slider_prev').on('click', function () {
			var slideObj = jQuery(this).parents('.stripe_gallery_wrapper').attr('data-uniqid');
...
function stripe_prevSlide(cur_slider) {
	var current_slider = jQuery('.stripe_gal_'+cur_slider);
