...
jQuery(document).ready(function ($) {
	if (jQuery('.circles_gallery_trigger').size() > 0) {
		...
		jQuery('.circles_prevSlide').on('click', function () {
			var slideObj = jQuery(this).parents('.circles_slider_wrapper').attr('data-uniqid');
...
function circles_prevSlide(slideObj) {
	var current_slider = jQuery('.circles_gal_'+slideObj);
	var cur_slide = parseInt(current_slider.find('.circles_current').attr('data-count'));
	cur_slide--;
...
function setup_circles(cur_slider) {
	if (jQuery('.circles_fs_on').size() > 0) {
		...
		var current_slider = jQuery('.circles_gal_'+cur_slider);
		...
		if (jQuery('#wpadminbar').size() > 0) {
			less_height = less_height + jQuery('#wpadminbar').height();
			var admin_bar = jQuery('#wpadminbar').height();
		}
		if (setHeight == '100%') {
