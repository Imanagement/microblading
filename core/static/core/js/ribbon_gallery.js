...
jQuery(document).ready(function ($) {
	if (jQuery('.ribbon_gallery_trigger').size() > 0) {
		...
		jQuery('.ribbon_prevSlide').on('click', function () {
			var slideObj = jQuery(this).parents('.ribbon_slider_wrapper').attr('data-uniqid');
...
function ribbon_prevSlide(slideObj) {
	var current_slider = jQuery('.ribbon_gal_'+slideObj);
	var cur_slide = parseInt(current_slider.find('.ribbon_current').attr('data-count'));
	cur_slide--;
...
function set_ribbon_Slide(slideNum, slideObj) {
	...
	if (r_max_slide < 5) {
		...
		if (parseInt(current_slider.attr('data-pad')) > 0) {
			var set_ribbon_pad = parseInt(current_slider.attr('data-pad'));
		} else {
			var set_ribbon_pad = 0;
		}

		var mainOffSet = current_slider.width()*0.5 - curSlide.width()*0.5;
...
function setup_ribbon(cur_slider) {
	...
	if (jQuery('.ribbon_fs_on').size() > 0) {
		...
		fs_ribbon_gal.height(setHeight).css('top', setTop+'px');
		fs_ribbon_gal.find('.ribbon_slider').height(parseInt(setHeight));
		fs_ribbon_gal.find('.ribbon_slide').height(parseInt(setHeight));
		...
		var current_slider = jQuery('.ribbon_gal_'+cur_slider);
