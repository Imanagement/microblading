...
if (jQuery('.fs_gallery_trigger').size() > 0) {
	jQuery(document.documentElement).keyup(function (event) {
		if (jQuery('.hovered_fsgal_module').size() > 0) {
			...
			if ((event.keyCode == 27)) {
				jQuery('.hovered_fsgal_module').removeClass('show_fs_share');
			}
			if ((event.keyCode == 39)) {
				event.preventDefault();
			}
...
jQuery(document).ready(function ($) {	
	...
	if (jQuery('.fs_gallery_trigger').size() > 0) {
		...
		jQuery('.fs_slider_prev').on('click', function () {
			var slideObj = jQuery(this).parents('.fs_gallery_wrapper').attr('data-uniqid');
		...
		jQuery('.fs_slider_controls').on('click', function () {
			var slideObj = jQuery(this).parents('.fs_gallery_wrapper');
		...
		jQuery('.fs_slider_share').on('click', function () {
			var slideObj = jQuery(this).parents('.fs_gallery_wrapper');
			slideObj.addClass('show_fs_share');
		});
		jQuery('.fs_share_block').on('click', function () {
			var slideObj = jQuery(this).parents('.fs_gallery_wrapper');
			slideObj.removeClass('show_fs_share');
		});
		jQuery('.fs_play_pause').on('click', function(){
			var current_slider = jQuery(this).parents('.fs_gallery_wrapper');
			var slideObj = jQuery(this).parents('.fs_gallery_wrapper').attr('data-uniqid');
...
function fs_prevSlide(cur_slider) {
	var current_slider = jQuery('.fs_gal_'+cur_slider);
	var cur_slide = parseInt(current_slider.find('.current-slide').attr('data-count'));
	cur_slide--;
...
function setup_fs_gallery(cur_slider) {	
	...
	if (jQuery('.fs_style_on').size() > 0) {
		...
		fs_gal.height(setHeight).css('top', setTop+'px').width(setWidth);
...
function onPlayerStateChange(event) {
	...
	if (yt_cut_state == 0) {
		if (jQuery('.fs_play_pause').hasClass('fs_state_play')) {
			var cur_slide_obj = jQuery('.current-slide');
			var cur_slide = parseInt(jQuery('.current-slide').attr('data-count'));
			var cur_slider = cur_slide_obj.parents('.fs_gallery_wrapper').attr('data-uniqid');
			var current_slider = cur_slide_obj.parents('.fs_gallery_wrapper');
			cur_slide++;
			max_slide = current_slider.find('.fs_slide').size();
			if (cur_slide > max_slide) cur_slide = 1;
			if (cur_slide < 1) cur_slide = max_slide;
