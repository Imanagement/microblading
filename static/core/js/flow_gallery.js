...
jQuery(document).ready(function ($) {
	if (jQuery('.flow_gallery_trigger').size() > 0) {
		...
		jQuery('.flow_prevSlide').on('click', function () {
			var slideObj = jQuery(this).parents('.flow_slider_wrapper').attr('data-uniqid');
...
function flow_prevSlide(slideObj) {
	var current_slider = jQuery('.flow_gal_'+slideObj);
	var cur_slide = parseInt(current_slider.find('.flow_current').attr('data-count'));
	cur_slide--;
...
function set_flow_Slide(slideNum, slideObj) {
	...
	if (r_max_slide < 5) {
		...
		current_slider.find('.flow_title').fadeOut(300, function () {
			setTimeout("current_slider.find('.flow_title').html(current_slider.find('.flow_current').attr('data-title'))",100);
			setTimeout("current_slider.find('.flow_title').fadeIn(500)",200);
		});
		current_slider.find('.flow_descr').fadeOut(300, function () {
			setTimeout("current_slider.find('.flow_descr').html(current_slider.find('.flow_current').attr('data-descr'))",100);
			setTimeout("current_slider.find('.flow_descr').fadeIn(500)",200);
		});
...
function setup_flow(cur_slider) {
	...
	if (jQuery('.flow_fs_on').size() > 0) {
		...
		var height_fixer = fs_flow_gal.find('.flow_title_content').height() + fs_flow_gal.find('.flow_gallery_trigger').height()*2;
		var setHeight = myWindow.height() - height_fixer,
			setTop = 0;
			
		if (jQuery('#wpadminbar').size() > 0) {
			setHeight = setHeight - jQuery('#wpadminbar').height();
			var setTop = setTop + jQuery('#wpadminbar').height();
		}
		if (fs_flow_gal.attr('data-toppx') !== '') {
			var setTop = setTop + parseInt(fs_flow_gal.attr('data-toppx'));
		}
		if (fs_flow_gal.attr('data-topcont') !== '') {
			if (jQuery(fs_flow_gal.attr('data-topcont')).size() > 0) {
				var setTop = setTop + parseInt(jQuery(fs_flow_gal.attr('data-topcont')).height());
			}
		}
		...
		fs_flow_gal.height(setHeight + height_fixer).css('top', setTop+'px');
		fs_flow_gal.find('.flow_slider').height(parseInt(setHeight)).css('top', setTop + fs_flow_gal.find('.flow_gallery_trigger').height() + 'px');
		fs_flow_gal.find('.flow_slide').height(parseInt(setHeight));
		...
		flow_slider_wrapper.each(function(){
			...
			height_fixer = jQuery(this).find('.flow_title_content').height() + jQuery(this).find('.flow_gallery_trigger').height()*2;
			if (setHeight == '100%') {
		...
		var current_slider = jQuery('.flow_gal_'+cur_slider);
		height_fixer = current_slider.find('.flow_title_content').height() + current_slider.find('.flow_gallery_trigger').height()*2;
